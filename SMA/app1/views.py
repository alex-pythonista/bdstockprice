...
def market_summary(request):
    if request.user.is_authenticated:
        try:
            print("Attempting to fetch market data...")
            global M_data
            obj = Today_Market()
...
def historic_data(request):
    if request.method == "POST":
        try:
            ...
            # df = get_hist_data(startdate,enddate,stockcompanies)
            ...
            global H_data
            H_data = get_hist_data(startdate,enddate,stockcompanies)
    ...
    if request.user.is_authenticated:
        try:
            ...
            if len(data) > 50:
                ...
                data = data.drop('ltp', axis=1)
                data = data.drop('ycp', axis=1)
